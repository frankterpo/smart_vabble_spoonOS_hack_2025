...
async function deploy() {
    console.log("==========================================");
    console.log("   DD REGISTRY - TESTNET DEPLOY");
    console.log("==========================================");
    ...
    const nefPath = path.join(projectRoot, "build", "DDRegistry.nef");
    const manifestPath = path.join(projectRoot, "build", "DDRegistry.manifest.json");

    if (!fs.existsSync(nefPath) || !fs.existsSync(manifestPath)) {
        throw new Error("Missing build artifacts. Run 'npm run compile:dd' first.");
    }
    ...
    try {
        console.log("🚀 Deploying DDRegistry...");
        ...
        console.log(`DD_REGISTRY_CONTRACT_HASH=0x${scriptHash}`);

        fs.writeFileSync(
            "./build/dd-deployment.json",
