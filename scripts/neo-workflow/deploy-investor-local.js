...
async function deploy() {
  try {
    console.log("🚀 Deploying InvestorShare to LOCAL neo-express node...");
    ...
    const nefPath = path.join(projectRoot, "build", "InvestorShare.nef");
    const manifestPath = path.join(projectRoot, "build", "InvestorShare.manifest.json");
    ...
    console.log("📦 InvestorShare Script Hash:", scriptHash);

    const deploymentPath = path.join(projectRoot, "build", "investor-deployment.json");
    fs.writeFileSync(
      deploymentPath,
      JSON.stringify({
        contract: "InvestorShare",
        txid: result,
    ...
    console.log("💾 Saved investor-deployment.json");
    console.log("");
    console.log("➡️  Add to .env: INVESTOR_SHARE_CONTRACT_HASH=" + scriptHash);
    console.log("");
