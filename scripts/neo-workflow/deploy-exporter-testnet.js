...
async function deploy() {
    console.log("==========================================");
    console.log("   EXPORTER REGISTRY - TESTNET DEPLOY");
    console.log("==========================================");
    ...
    const nefPath = path.join(projectRoot, "build", "ExporterRegistry.nef");
    const manifestPath = path.join(projectRoot, "build", "ExporterRegistry.manifest.json");

    if (!fs.existsSync(nefPath) || !fs.existsSync(manifestPath)) {
        throw new Error("Missing build artifacts. Run 'npm run compile:exporter' first.");
    }
    ...
    try {
        console.log("🚀 Deploying ExporterRegistry...");
        ...
        console.log(`EXPORTER_REGISTRY_CONTRACT_HASH=0x${scriptHash}`);

        // Save deployment info
        fs.writeFileSync(
            "./build/exporter-deployment.json",
            JSON.stringify({ txid: result, scriptHash: `0x${scriptHash}` }, null, 2)
        );
        console.log("💾 Saved to build/exporter-deployment.json");
