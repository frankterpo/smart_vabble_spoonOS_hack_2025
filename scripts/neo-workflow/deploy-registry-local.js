...
async function deploy() {
  try {
    console.log("🚀 Deploying ReceivableRegistryV2 to LOCAL neo-express node...");
    ...
    const nefPath = path.join(projectRoot, "build", "ReceivableRegistryV2.nef");
    const manifestPath = path.join(projectRoot, "build", "ReceivableRegistryV2.manifest.json");
    ...
    console.log("📦 ReceivableRegistryV2 Script Hash:", scriptHash);

    const deploymentPath = path.join(projectRoot, "build", "registry-deployment.json");
    fs.writeFileSync(
      deploymentPath,
      JSON.stringify({
        contract: "ReceivableRegistryV2",
        txid: result,
    ...
    console.log("💾 Saved registry-deployment.json");
    console.log("");
    console.log("➡️  Add to .env: RECEIVABLE_REGISTRY_V2_CONTRACT_HASH=" + scriptHash);
    console.log("");
